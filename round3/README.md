...
Per price point, both strategies give identical profit (4201). The counterparty reserve prices follow a uniform distribution, so in expectation there should be an equal number of counterparties at each price point. We treated this as exact in our model. In practice though, the actual number of counterparties at each price point varies randomly around that expectation. By bidding 766 instead of 761, we moved the pods at the 765 reserve price from bid 2 (earning 59 each) to bid 1 (earning 154 each). That sounds good, but it turns out there were only about 15 counterparties at 765, while other price points in the $b_2$ range had more. The uneven draw slightly favored their split over ours.

The gap is only 340 though. Essentially noise from the random counterparty draw. Our $b_1$ adjustment to 766 was designed to be free insurance: with equal counterparties at each price point (the expected case), both (761, 861) and (766, 861) yield the exact same profit when the average lands at or below 861. The only reason it "cost" us anything is that the random draw happened to put fewer counterparties at the one price point where our strategies differ.
